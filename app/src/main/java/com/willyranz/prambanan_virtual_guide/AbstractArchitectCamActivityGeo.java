 ...
 */
public abstract class AbstractArchitectCamActivityGeo extends Activity implements ArchitectViewHolderInterfaceGeo {
    /**
        ...
        this.locationListener = new LocationListener() {
            ...
            public void onLocationChanged(final Location location)
            {
                ...
                if (location != null)
                {
                    ...
                    AbstractArchitectCamActivityGeo.this.lastKnownLocaton = location;

                    if (AbstractArchitectCamActivityGeo.this.architectView != null)
                    {
                        // check if location has altitude at certain accuracy level & call right architect method (the one with altitude information)
                        if (location.hasAltitude() && location.hasAccuracy() && location.getAccuracy() < 7) {
                            AbstractArchitectCamActivityGeo.this.architectView.setLocation(location.getLatitude(), location.getLongitude(), location.getAltitude(), location.getAccuracy());
                        } else {
                            AbstractArchitectCamActivityGeo.this.architectView.setLocation(location.getLatitude(), location.getLongitude(), location.hasAccuracy() ? location.getAccuracy() : 1000);
                        }
                    }
                }
            }
    ...
    {
        int features = StartupConfiguration.Features.Geo;
        return features;
    }
    ...
    protected void onPostCreate( final Bundle savedInstanceState ) {
        ...
        if ( this.architectView != null ) {
            ...
            try {
                ...
                if (this.getInitialCullingDistanceMeters() != ArchitectViewHolderInterfaceGeo.CULLING_DISTANCE_DEFAULT_METERS) {
                ...
                public void run() {
                    ...
                    while (lastKnownLocaton==null && !isFinishing()) {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            @Override
                            public void run() {
                                Toast.makeText(AbstractArchitectCamActivityGeo.this, "Trying to find out where you are", Toast.LENGTH_SHORT).show();
                            }
                        });
    ...
    public static JSONArray getPoiInformation(final Location userLocation,  final int numberOfPlaces)
    {
        ...
        for (int i=1;i <= numberOfPlaces; i++) {
            ...
            poiInformation.put(ATTR_NAME, "Prambanan" + i);
            poiInformation.put(ATTR_DESCRIPTION, "Prambanan description" + i);
