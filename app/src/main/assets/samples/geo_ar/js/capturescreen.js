...
var World = {
	...
	*/
	...
	/*
		This sample shows you how to use the function captureScreen to share a snapshot with your friends.
		Concept of interaction between JavaScript and native code is same as in the POI Detail page sample but the urlListener now handles picture sharing instead.
	...
	*/
	...
	// load dummy places from content source
	loadDummy: function loadDummyFn() {
		if (!World.isRequestingData) {
			if (World.userLocation) {
				World.requestDataDummy(World.userLocation.latitude, World.userLocation.longitude);
			} else {
				World.updateStatusMessage('Unknown user-location.', true);
			}
		} else {
	...
	// request POI dummy data
	requestDataDummy: function requestDataDummyFn(lat, lon)
	{
	...
	*/
	bringPlacesToUser: function bringPlacesToUserFn(poiData, latitude, longitude) {
		...
		for (var i = 0; i < poi.length; i++) {
			poi[i].latitude = latitude + (Math.random() / 5 - 0.1);
			poi[i].longitude = longitude + (Math.random() / 5 - 0.1);
			poi[i].altitude = AR.CONST.UNKNOWN_ALTITUDE;
		}
		return poi;
	}
}
